...
class IndexController extends Controller {
	public function index() {
		//		dump(C('DEFAULT_THEME'));
		//		$this->show("<a href=".U('Jjy/Home/index').">index</a>");
		redirect(U('Jjy/Public/login'));
	}
		...
		foreach ($data as $ke => $row) {
			...
			foreach ($row as $key => $rows) {
				...
				if (count($row) == 1 && empty($row[0][1]) && empty($rows[1]) && !empty($rows)) {
					...
					$objPHPExcel -> getActiveSheet() -> setSharedStyle($sharedStyle1, "A{$column}:J{$column}");
					//设置边框
					array_unshift($rows, $rows['0']);
					$objPHPExcel -> getActiveSheet() -> mergeCells("A{$column}:J{$column}");
					//合并单元格
					$objPHPExcel -> getActiveSheet() -> getStyle("A{$column}:J{$column}") -> getFont() -> setSize(12);
					//字体
					$objPHPExcel -> getActiveSheet() -> getStyle("A{$column}:J{$column}") -> getFont() -> setBold(true);
					//粗体
